...
if (import.meta.url.endsWith(process.argv[1])) {
  ...
  try {
    await verifyChangesets(pullRequestEvent, process.env, fs.readFile);
    await fs.writeFile(
...
export async function verifyChangesets(
  ...
  if (hasMinorReleaseLabel) {
    throw Object.assign(
      new Error("Skipping changeset verification - minor-release label found"),
      {
        path,
      },
    );
  ...
  for (const path of env.CHANGED_FILES.trim().split(" ")) {
    ...
    if (invalidVersionBumps.length > 0) {
      throw Object.assign(
        new Error(
          `Invalid .changeset file - invalid version bump (only "patch" is allowed, see <DOCS URL>)`,
        ),
